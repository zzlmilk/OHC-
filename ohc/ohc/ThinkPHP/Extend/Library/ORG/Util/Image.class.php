...
class Image {
     ...
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=65, $height=28, $verifyName='verify') {
...
//        $backColor = imagecolorallocate($im, $r[$key], $g[$key], $b[$key]);    //背景色（随机）
         $backColor = imagecolorallocate($im, 161,161, 161);    //背景色（随机）
       // $borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
        imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        //imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
...
//        }
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 5, $i * 10 + 12, 7, $randval{$i}, $stringColor);
        }
